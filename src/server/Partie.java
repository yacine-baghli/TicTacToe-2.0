...
public class Partie extends Thread {
    ...
    private boolean proMode = false;//TODO ajouter à la partie ou y a la commande /inverser pour la bloquer
...
//    //TODO à ajouter
    ...
    private static final String JOUE_COMME_UN_PRO=ETOILE+"\n"+NomNiveau+"\nQu'est ce que tu essaies de faire? Joue comme un pro!"+ETOILE;
    
    private static final String HACKER_MODE="\n"+NomNiveau+"HAHA bien essayé mais tu ne t'en sortiras pas comme ça!"+ETOILE;
    private static final String TU_PERDS_DEJA="\n"+NomNiveau+"Ptdr c'était rapide"+ETOILE;
    private static final String JAI_TON_IP=ETOILE+"\nTu as triché! Ce n'est pas possible autrement!\nJ'ai ton IP de toute façon..."+ETOILE;
    private static final String JAI_TON_IP_SUITE=ETOILE+"\nAh tu ne me crois pas? MDR ça te dit quelquechose ça :"+getIP()+"?"+ETOILE;
...
//	        	Difficulte=0; //TODO Verifier
    ...
    private void jouerUnePartie() {
        ...
        String joueurNom = (joueur == 1) ? joueur1Name : NomNiveau;
        
        // Afficher le message du premier joueur
    	if (modeDeJeu == MODE_SOLO)envoyerMessage(out1, ETOILE + "\nC'est au joueur " + joueurNom + " de commencer\nla partie. Bonne chance !" + ETOILE);
    	else if (modeDeJeu == MODE_DUO) {
    		
    		joueurNom = (joueur == 1) ? joueur1Name : joueur2Name;
    		envoyerMessage(out1, ETOILE + "\nC'est au joueur " + joueurNom + " de commencer\nla partie. Bonne chance !" + ETOILE);
            envoyerMessage(out2, ETOILE + "\nC'est au joueur " + joueurNom + " de commencer\n la partie. Bonne chance !" + ETOILE);
    ...
    public void jouerTourBot(){
    	...
    	envoyerMessage(out1, String.format(OPPONENT_TURN_MSG, NomNiveau, "Tour du joueur "));
    ...
    public void tourPro() {
        ...
        if (nbTours == 0 || (nbTours == 1 && estCoin(derniereLigne, derniereColonne))) {
            // Premier tour de la partie: placer le symbole au centre
            iBot = 1;
            jBot = 1;
        } else {
            // Les tours suivants : vérifier les lignes, colonnes et diagonales
            caseBV = plateau.checkBloquageOuVictoire();
            
            if (caseBV[0] != -1 && caseBV[1] != -1) {
                // Si le bot peut gagner, compléter la série
                // Si le bot peut perdre, bloquer l'adversaire
                iBot = caseBV[0]; // Correction de l'indice pour la ligne
                jBot = caseBV[1]; // Correction de l'indice pour la colonne
            } else {
    ...
    private boolean estCoin(int ligne, int colonne) {
        return (ligne == 0 && colonne == 0) || // Coin supérieur gauche
               (ligne == 0 && colonne == 2) || // Coin supérieur droit
               (ligne == 2 && colonne == 0) || // Coin inférieur gauche
               (ligne == 2 && colonne == 2);   // Coin inférieur droit
    }
        ...
        try {
            ...
            if (i == 49 && j == 3) {
                ...
                if (message.equals("/jegagnesijeveux")) {
                	...
                	if((modeDeJeu==MODE_SOLO)&&(hackerMode)) {
                    
                		envoyerMessage(out,HACKER_MODE);
	                ...
	                if((modeDeJeu==MODE_SOLO)&&(hackerMode||proMode)){
                		envoyerMessage(out,JOUE_COMME_UN_PRO);
	                }
	                else{
            ...
            if (i == 99 && j == 99) {
                ...
                if (message.equals("/matchnul")) {
                	...
                	envoyerMessage(out, "Vous venez de passer votre tour...");
                	if (modeDeJeu==MODE_DUO)envoyerMessage(opponentOut, "Votre adversaire a passé son tour...");
            ...
            if((modeDeJeu==MODE_SOLO)&&(joueur==1))derniereCase(i, j);
...
//            else stopPartieSolo(RAISON_AVEC_OPTIONS_SOLO);
        ...
        if (modeDeJeu==MODE_DUO)stopPartieDuo(RAISON_AVEC_OPTIONS);
        else if (modeDeJeu==MODE_SOLO)stopPartieSolo(RAISON_AVEC_OPTIONS_SOLO);
    }
    ...
    private void stopPartieDuo(String raison) {
        ...
        try {
        	...
        	while(!choix1.equals(choix2)) {
        		...
        		envoyerMessage(out1, "Veuillez vous mettre d'accord avec votre\nadversaire (Vous pouvez utiliser\nla commande /msg pour envoyer un message\nà votre adversaire)");
        		choix1 = in1.readLine();
        		
        		envoyerMessage(out1, "Veuillez attendre que votre adversaire fasse un choix...");
        		envoyerMessage(out2, "Veuillez vous mettre d'accord avec votre\nadversaire (Vous pouvez utiliser\nla commande /msg pour envoyer un message\nà votre adversaire)");
        		choix2 = in2.readLine();
        	}
    ...
    private void stopPartieSolo(String raison) {
        ...
        try {
            ...
            } else if(choix1.equals("4")) {
                
            } else {
            	stopPartieSolo(RAISON_AVEC_OPTIONS_SOLO);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
